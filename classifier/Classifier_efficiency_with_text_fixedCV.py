...
LOG_FILENAME = datetime.now().strftime('LogFile_100w2v_scale_newCV_no_Peff_Split_TimeComment_%d%m%Y%H%M.log')
...
class Classifier:
    def __init__(self):
        ...
        print('{}: Loading the data: FinalFeatures_with_comment_time'.format((time.asctime(time.localtime(time.time())))))
        self.original_data = pd.read_excel('FinalFeatures_with_comment_time.xlsx')
        self.labels = None
        ...
        self.comment_features = ['comment_created_time_hour', 'time_between_messages', 'comment_len',
                                 'number_of_r', 'number_of_references_to_submission']
...
###############################################################################
    def split_relevant_data(self, Peff_up_threshold, Peff_down_threshold):
        ...
        if Peff_up_threshold == 66.0 or Peff_up_threshold == 100.0:
            opts.k_fold = 5
...
# benchmark classifiers
    def benchmark(self, clf, clf_name='default'):
        ...
        print("Accuracy: {}".format(average_acc))
        logging.info("Accuracy: {})".format(average_acc))

        average_auc = sum(auc)/len(auc)
        print("AUC: {}".format(average_auc))
        logging.info('AUC: {}'.format(average_auc))
...
if __name__ == '__main__':
    ...
    threshold_list = [0.0, 20.0, 50.0, 100.0]
    ...
    for i in range(len(threshold_list) - 1):
        ...
        main_all_groups_results = classifier.iterate_over_features_groups(threshold_list[i + 1], threshold_list[i])
        all_Peff_groups_results = all_Peff_groups_results.append(main_all_groups_results, ignore_index=True)

    classifiers_list = all_Peff_groups_results.classifier_name.unique()
    groups_list = all_Peff_groups_results['group_list'].tolist()
    groups_list.sort()
    ...
    for index, row in all_Peff_groups_results.iterrows():
        classifiers_results_dict[(row['classifier_name'], tuple(row['group_list']))][1].append(row['auc'])
        classifiers_results_dict[(row['classifier_name'], tuple(row['group_list']))][0].append(row['score'])
    ...
    all_Peff_groups_results.to_csv('test_results_final_split_no_Peff_with_time_50_7_11.csv', encoding='utf-8')
